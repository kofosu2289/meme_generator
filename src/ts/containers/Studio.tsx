  ...
  return (
    <div className="Studio">
      ...
      <aside className="Studio__aside">
        {/* TODO: each time we update the state, it calls the api, need to store meme urls */}
        <div className="buttons__actions">
          <button className={state === 'memes' ? 'active' : null} onClick={(): void => setState('memes')}>
            Memes
          </button>
          <button className={state === 'edit' ? 'active' : null} onClick={(): void => setState('edit')}>
            Edit
          </button>
        </div>
        {state === 'memes' ? (
